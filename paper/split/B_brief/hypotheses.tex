...
followers learn fastest. & Share of a theme's first five years of
filers in its top state; expect a larger penalty in concentrated
themes. & build (state only) \\
29 & \emph{Distance from the cluster.} Early filers far from a theme's
home state are shielded from local imitation, or cut off from its
resources. & Within theme, filers in the theme's hub state against
filers elsewhere; the sign is the question. & build (state only) \\
...
lines up with each literature, not a search for significance. City-level
geography would need owner addresses re-parsed from the raw case files;
the state-level version is ready.
