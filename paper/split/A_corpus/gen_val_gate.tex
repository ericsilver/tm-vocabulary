...
firms, is the subject of the companion paper.

\subsubsection{Why the analysis conditions on registration}\label{sec:filter}
...
considered launching, including those who then did not. And since they
are 2.3 million of the 6.0 million scored applications, including them
would also bury the commercial signal in launch noise.
...
\subsubsection{Leading marks fail the five-year proof more often}\label{sec:gate_headline}
...
counsel or by the owner is controlled for because it moves both sides of
the comparison at once: self-filers fail the five-year proof at 69.6\%
against 46.8\% for represented owners, and
Section~\ref{sec:representation} showed they write shorter, more
class-typical, slightly more leading descriptions. Without the control,
part of any lead gradient would really be a self-filing gradient. With it, the penalty is of the same order in both
...
$+10.3$pp.} About forty percent of the raw pooled contrast is therefore
composition --- differences between industries, cohorts and filer types
--- rather than the within-cell gradient: $+1.4$pp design-based against
$+2.5$pp raw. The penalty is also not
...
The companion paper takes that variation as its subject.
...
Two mechanical readings of the penalty are checked and fail. The first
is timing: if leading marks merely failed on a different schedule, a mark
failing outside the 4.0--8.5 window would count as a survivor and the
window would manufacture the contrast. In fact the proof is close to a
fixed appointment: 96.1\% of cancellations post at registration age
6.5--7.0. On the 3.15 million registrations whose ninth anniversary is
inside the record --- where failure is settled and no window is needed
--- the contrast is $+2.71$pp against the window's $+2.65$, and among
marks that fail, failing late shows no lead gradient at all ($-0.01$pp,
$t = -0.2$). The second is disengagement: perhaps the proof measures an
owner who lost interest in the paperwork rather than a product that left
the market. But passage is flat across office-action response-speed
quintiles (46.1\% to 48.1\%, from a median response of seven days to one
of 183), and the penalty is present in both speed halves ($-3.48$ and
$-2.36$pp on passing, against $-2.94$ pooled).
The supplementary material carries the detail, including why 2019 is the
last observable cohort.

\subsubsection{The penalty under three theme scorings}\label{sec:three_scorings}
...
levels whose average is atypicality, so the spread of lead grows with
atypicality, and lead \emph{magnitude} correlates with atypicality at
$+0.29$. But the signed value does not ($-0.05$), and cutting lead
quintiles within fifths of atypicality leaves the penalty intact in
every fifth: $+2.1$, $+3.1$, $+3.3$, $+1.7$ and $+2.5$pp from the least
atypical fifth to the most (each SE under $0.3$).

\subsubsection{The risk persists at the year-ten renewal}\label{sec:renewal}
...
five-year proof, $-0.63$pp on renewing at ten (harmonized estimates in the companion paper,
...
\subsubsection{What the five-year proof shows}\label{sec:gate_conclusion}
