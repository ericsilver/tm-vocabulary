...
Resolution, 50 vs 200 themes\footnotemark              & $0.636$ & $0.799$ \\
...
sum-and-difference construction of Section~\ref{sec:construct} implies ---
atypicality is a property of the filing, and the weighting acts almost
entirely on the small residual that is lead. Resolution and representation matter
...
test, which is why the term-scored replication in \S\ref{app:scoring}
...
$T = 50$ sits at the low end of the range rather than the high one. (A
fifth fit, at $T = 1000$, failed a training-convergence check and is
excluded.) And
...
The corpus-wide contrast is steadier than the single class: scored for all
45 classes, it is $+2.29$pp at $T = 50$, $+2.23$pp at $T = 200$ and
$+2.61$pp at $T = 500$. Pooling evidently averages away much of the
per-class resolution sensitivity.
...
estimates, not shrink them. And the seed explains most of the disagreement
in per-filing scores between resolutions, but not the spread of the
class-level estimates: the seed moves the software-class contrast by a
third of a point, while the resolutions in the table above span three.
That spread reflects the scorings disagreeing about which filings are
leading, and pooling across classes largely averages it out.

The same test at the production resolution gives a split answer. A
second fit at $T = 50$, again differing only in its seed, recovers the
themes themselves far more faithfully than at $T = 200$: matching each
theme to its closest counterpart by the cosine of their word
distributions, the median match is $0.74$ at fifty themes against $0.54$
at 200, and a quarter of the fifty themes match above $0.9$. Per-filing
scores are no more stable for it: the two fits agree at $r = 0.72$ on
lead and $0.84$ on atypicality across the same 1.11 million
software-class filings, and the five-year-proof lead contrast is
$+3.23$pp under the first seed and $+3.89$pp under the second (SE $0.23$
each). A coarse model's word lists are reproducible; which theme a
borderline filing's words are credited to is not, and that is where the
per-filing noise comes from.
...
correlate at $0.980$ on the 3.10 million five-year-proof registrations and atypicality
at $1.000$ to three decimals. On the identical registrations, with quintiles
cut within class and registration year, the top-minus-bottom proof contrast
...
The window is a parameter of the measure, not a property of it, and the
five years used throughout was fixed early in the project rather than
tuned to the present construction. To test it, we re-score the software
class at several widths under per-filing references and judge each width
by the five-year-proof quintile contrast it produces: $+2.02$pp at two
years, $+2.20$ at three, $+3.23$ at five and $+3.37$ at seven, each on an
identical sample of 452{,}911 registrations and each with a standard
error of $0.23$pp. Signal
...
proof sample, dropping the earliest and latest registration years, which are
...
only the next three. We score every class on the production
representation and per-filing windows at $W \in \{3, 5, 7\}$ years on
each side, and evaluate all five variants on identical samples. In the
table, the proof columns are computed on the 3.10 million registrations
2002--2018, with quintiles cut within class$\times$registration-year;
the registration columns on the scored filings 1995--2018, with
quintiles within class$\times$filing-year:

\begin{center}\small
\begin{tabular}{llrrrr}
\toprule
Variant & Past, future & \multicolumn{2}{c}{Five-year-proof failure} & \multicolumn{2}{c}{Registration} \\
...
$+1.82$ and $+5.07$pp. So the five-year proof punishes distance from the
...
Section~8 proof by the record's end in April 2026; the earliest registrations in the
window also face their year-ten renewal, so panel B mixes one and two
proofs). Panel A is the unconditional composite, the probability that a
filing both registers and passes the proof, measured over all filings
including the never-registered. It is nearly flat in lead because two
opposed gradients cancel: registration rises in lead
($61.3$\% to $63.2$\%) while proof survival falls ($44.2$\% to $40.8$\%);
the caption carries the quintile figures.
Conditioning on grant is what separates them. Panel B shows the conditional
proof outcome on the same filings, and it is U-shaped rather than inverse-U
(depth $-2.11$pp), with a clearly penalised leading tail consistent with the
cleaner single-proof cohort of Figure~\ref{fig:s8pooled}.
...
per-filing reference windows ($n = 1{,}419{,}678$; proof-facing subset
...
and facing the proof) on the same filings. The unconditional view is nearly flat in
...
reverses a headline claim; three auxiliary readings shift.
...
penalty at the proof belongs to the 2010--2018 cohorts; the 2002--2009 half
...
coupling under both representations. The consequence for lead is direct:
the standard deviation of $L$ rises $2.2$--$2.5$-fold across deciles of
$A$ under term scoring and $1.5$--$1.8$-fold under theme scoring, so a
unit of lead means roughly the same thing everywhere only in the theme
representation. \S\ref{app:representation} shows this as a length
surface over the two axes.

One qualification: ``flat'' means no monotone
length gradient rather than constant: class 035 under theme scoring is
mildly U-shaped in $A$. The two scorings agree only moderately per
filing (Pearson $0.46$, Spearman $0.42$ on the 6.01 million filings both
score). That is what gives their agreement on the mark-level patterns
evidential force, and their disagreement on the firm-level patterns
diagnostic force (Section~\ref{sec:robust_hazard}). Across
...
uncorrelated with lead on the same panel (Pearson $+0.010$).

\subsection{Term scoring as a cross-check}\label{app:scoring}

The snapshot cross-check on the single one-proof-elapsed cohort
...
scoring on the identical sample (Figure~\ref{fig:s8pooled}). Under term scoring, proof survival
\emph{rises} in the raw KL level ($+6.5$pp past-facing, $+7.6$pp
future-facing): atypical text survives the five-year proof better
...
scoring (depth $-0.001$). The proof result is scoring-robust; the
...
\caption{Binned profiles at the three steps, with the linear reading the
...
distance from a vertex. A V beating a parabola says the turn is sharper
than a quadratic allows, not that anything grows exponentially.
...
and the $12$ to $36$ at the public-reporting steps are decisive; the $1.9$ at
...
shifted left, which is where the fits put them ($+0.24$ to $+0.41$
standard deviations for the reporting minima; the survival vertex, at
$-1.5$, comes from a weak fit, $\Delta$AIC $4.3$).
...
Two dependences run through the corpus. Filings can share an exact
position, because identical text carries identical scores. And outcomes
cluster within owner: the within-owner correlation is 0.38 for proof
survival pooled, and it is mechanically 1.0 for public listing, since
listing is a property of the owner and a firm with hundreds of marks
contributes hundreds of identical successes. The regression tables are
unaffected, because the proof regressions cluster errors on normalized
owner and the firm-level specifications carry one observation per owner.
The raw quintile contrasts of the companion paper --- the era, theme,
internet and surge tables --- are not: their two-sample standard errors
ignore the owner clustering, and their $t$-statistics overstate
precision by roughly a factor of two, which the text of those sections
carries where the margins are close. Binning filings without the
correction likewise overstates how much structure the corpus contains.
On a positional grid, correcting cell errors for the clustering design
effect --- the factor by which within-owner correlation inflates the
variance of a cell mean --- cuts apparent excess dispersion by roughly a
quarter on the proof outcome and by half on listing. Displays of the
vocabulary plane are descriptive here for that reason; and since the
near-zero pooled correlation between $A$ and $L$ reported in
Section~\ref{sec:construct} does not survive cell by cell within class,
per-cell contrasts should not be read as within-class effects.
