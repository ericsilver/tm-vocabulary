...
Section~8 proof by the 2025 snapshot; the earliest registrations in the
...
Two qualifications. Theme scoring returns a finite value for about 65\%
of filings, so the two columns are not drawn from the same set;
re-estimating the term-scored correlations on exactly the theme-scored
subset barely moves them, which puts the contrast down to the
representation rather than the sample. And ``flat'' means no monotone
