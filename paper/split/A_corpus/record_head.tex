...
prosecution history is a list of coded events with dates. We
re-parse every case file into 242 million dated events, keyed by serial
number. For each application, the full sequence is then available: office
actions, responses, allowances, statements of use,
registration, Section~8 and Section~9 filings, cancellations and
renewals, each with its
...
The parse also yields, for each filing, the median number of days the
applicant took to answer an office action. We retain this response
latency as an indicator of how attentively an application was
prosecuted.
