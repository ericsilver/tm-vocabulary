...
is 0.9 points with no consistent direction: filings written in language
their class was moving toward register at nearly the same rate as
filings written in language it was leaving.
...
them and without them. The supplementary material reports the pooled
analysis for a skeptical reader; its gradients are attenuated and in
...
specific way. Self-filers fail the proof at 69.6\% against 46.8\% for
...
counselled), and the estimate is if anything largest in the cleanest
stratum, counsel-represented US-domiciled owners. About forty percent of
...
is close to a fixed appointment (96.1\% of cancellations post at
registration age 6.5--7.0), and on the 3.15 million registrations whose
...
per-class themes (SE $0.09$ each), positive under all three at once in
...
every scoring; the atypicality gradient does not survive per-class
themes.}
...
vocabulary, and the imported theme becomes local. Measuring theme
arrival directly separates two kinds of new: registrations carrying a
theme new to their class (established elsewhere, arriving here) fail
the proof $0.43$ points \emph{less} within class and year --- but
$1.06$ points \emph{more} once their own lead and atypicality are held.
A theme arrives in a class in filings that are atypical for it, and
atypical filings survive; among filings of equal atypicality, the ones
carrying the arriving theme fail more. The apparent protection of
atypicality is mostly the protection of extension, and themes new to
the whole corpus are too rare to judge (779 registrations). The penalty
attaches to being early with language, not to being far from the
class's centre.
...
independently. On 94{,}181 firm-years across 17{,}506 trademark owners
name-matched to PatentsView assignees, regressing each axis on
$\log(1 + \mathrm{patents})$ with firms absorbed and errors clustered on
the firm puts every coefficient below $0.06$ standard deviations, and
the small cells flip sign between theme and term scoring. No timing
offset from $t-2$ to $t+1$ isolates a direction, and among owners who
...
complement and substitute regimes shows no interaction (detail in the
...
split accordingly below. Raw rates rise 2.7-fold across atypicality
...
atypicality keeps the structure, so the U is not the mechanical
coupling of the two axes. Splitting matched owners on whether their SEC
...
name-match linkage bounds the atypicality reading --- distinctive names
match more easily, and name distinctiveness plausibly travels with text
distinctiveness --- and the lead coefficient is the one unchanged under
...
real signal. Each filing's invisible-vocabulary share is itself a
strong firm marker --- but \emph{within} strata of equal invisible
...
split is the test that settles the question (full treatment in the
