...
Base failure rate 52.3\% on this sample. The same estimates appear in
the companion paper, where they validate the measure
...
cuts the magnitude by about forty percent without absorbing it --- from
$+2.3$pp with fixed effects alone to $+1.4$pp --- and the estimate is
present in the cleanest stratum, counsel-represented US-domiciled owners
($+1.6$pp), so it is not carried by the self-filed or foreign
mass-filing segments whose failure rates are high for their own
...
Per-class estimates, with each class's own profile, are in the online
appendix.\footnote{\onlineappendixnote}

The penalty is a rate, not a timing, and not disengagement. The proof is
close to a fixed appointment (97\% of in-window cancellations post at
...
Every estimate above scored filings on fifty themes fitted once for the
whole corpus, with each class supplying its own reference. The companion
paper rescores the identical registrations under \emph{500 global themes}
(a theme closer to a product category than to an industry) and under
\emph{50 themes fitted separately inside each class} (nothing shared
across classes), and the penalty does not depend on the choice: $+2.29$pp
under the production scoring, $+2.61$pp at 500 themes and $+1.94$pp
under per-class themes, positive under all three at once in 28 of the 44
classes with enough volume, even though the scorings agree only
moderately on which filings are leading (lead correlates at $0.64$ and
$0.52$ between them) \citep{SilverCorpus2026}.

Atypicality behaves differently.
...
arriving theme fail more; that is the $+1.06$. Together with the
collapse of the atypicality gradient under per-class themes, this
suggests the protection the global scorings show is largely a reading
of themes imported from other classes. The penalty attaches to being
early with language, not to being far from the class's centre.
...
at year ten, and on the harmonized estimates of Figure~\ref{fig:staged}
the lead penalty persists there at about half its size: $-0.97$pp per
standard deviation on passing the five-year proof, $-0.43$pp on renewing
at ten, both stated on survival. Pooled across classes the renewal
profile is a V rather than a slope --- renewal is highest just below the
centre of the lead distribution and lowest at both tails, the lagging tail
lowest of all (45.5\% against 50.6\% at the most leading). Most of the
difference between leading and lagging resolves at the five-year proof;
what remains at renewal is a penalty on both extremes, the lagging one
the larger.
...
opposite, but that protection largely disappears when themes are fitted
within each class, and filings carrying a theme imported from another
class fail more than equally atypical filings. The two steps the record
dates respond to the text differently: registration responds to distance
from the class's usual language at a moment when lead does not yet exist
to be read (Section~\ref{sec:reg_rates}); the five-year proof is recorded
after the industry's language has moved, and it is where being early
shows up as a cost.
